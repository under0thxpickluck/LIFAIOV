...
**開催期間**: 2026-08-01 00:00 〜 2026-08-14 23:59（2週間、ソフトクローズで最大3回延長）

**参加対象**: `approved` ステータスの全会員

**ルール**:
- 投入額: 1,000BP 単位、1人あたり上限 **1,000,000BP**
- EP還元: 確定 1000BP → 1EP（誰も"損"をしない。テスト期間中はEP上限なし）
- 乱数なし: 純粋に投入額で順位が確定
- 特典:
  - **Tier 1** (上位5名) — LIFAI新システム体験会優先参加権＋ベータテスター意見交換会参加券
  - **Tier 2** (6〜15位) — ベータテスター意見交換会参加券
- 体験会/意見交換会実施日時: 2026-08-25 〜 2026-09-07（別途日程調整）
- 当選通知: 2026-08-15 以降、**管理者が手動で Gmail/公式LINE で送付**（自動メール機能は本番化時に追加）

**ソフトクローズ仕様（駆け込み対策）**:
- 終了予定時刻まで残り 10分以内に投入があり、かつ **Tier 1 の5位ボーダーが変動したら** 終了時刻を 10分延長
- 延長は最大 **3回まで**（無限ループ防止）
- 判定は投入ごとに実行（投入前後のランキング Tier 5位を比較）

**フロント実装**:
- ログイン直後にポップアップ表示（期間限定・全approved会員対象）
  - 「【期間限定】BP貢献度オークション開催中！」というメッセージ + 「詳細ページへ」ボタン
  - ブラウザ sessionStorage に "auction_popup_shown" を記録し、セッション中は1回だけ表示
- `/market/auction` — オークション一覧・詳細ページ
  - 透明性ガード: ルール・特典・ボーダー・カウントダウン・「投入したBPは戻りません」を常時明示
  - 投入前に規約同意チェック（既存 narasu terms チェックUI と同様。4項目: 投入BP戻らない / 1000BP=1EP確定 / 乱数なし投入額順 / 景品非転売）
  - 投入フォーム＋リアルタイムランキング（上位20位、3秒ごと自動更新）
  - 自分のスコア・現在順位・ボーダー表示（「あと ◯◯ BP で Tier 2」等）

**GAS実装（テストレベルMVP）**:
- `LockService` でログイン_id 単位ロック（投入の原子性・二重消費防止）
- `auction_bids` に全投入を append-only で記録
- `wallet_ledger` に `kind="auction_bid"` / `kind="auction_ep_return"` で二重記帳（投入消費 + EP還元）
- `auction_close` アクションで終了判定→ティア割り当て確定→当選者リスト生成（メール手動送付用）
- **注記**: EP上限チェック・複雑な権限分離・自動メール機能は本番化（9月以降）で追加
