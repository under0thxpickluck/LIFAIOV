...
### レート計算式の変更（2026-06-22 追記・最大レート方式）

旧式 `基本レート = min(プール ÷ 総ステーク, 100%)` は総ステークがプール以下の間ずっと100%に張り付き、さらに倍率（×5など）で500%等の過大なレートになっていた。これを「最大レート方式」に変更:

```
消費率   = 総ステーク額 ÷ プール        （0〜1。プール上限ブロックにより最大1）
確定レート = (1 − 消費率) × 最大レート    （下限レートで底支え）
```

- `最大レート`（旧 multiplier。列名は後方互換で `multiplier` のまま）＝プールが空（消費率0）のときのレート。
- **既定値**: 30日 最大2% / 60日 最大5% / 90日 最大10%、下限レートはすべて 0%。
- 管理画面の入力欄は「最大レート（%）」「下限レート（%）」。内部では小数で保存（10% → 0.10）。

| 消費率 | 90日（最大10%） | 60日（最大5%） | 30日（最大2%） |
|---|---|---|---|
| 0%（空） | 10% | 5% | 2% |
| 50% | 5% | 2.5% | 1% |
| 100%（満杯） | 0% | 0% | 0% |

- プール未設定（pool=0）: 下限レートのみ（既定0%）。
