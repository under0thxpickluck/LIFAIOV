...
**Goal:** `/album-guide` に静的な隠しページを新規作成し、LIFAIユーザーがnarasu配信申請に必要なアルバム情報を5ステップで準備できるようにする。

**Architecture:** `app/album-guide/page.tsx` を1ファイル新規作成するだけ。サーバーコンポーネント（`"use client"` 不要）。既存ページへの変更なし。
...
| 操作 | パス | 内容 |
|------|------|------|
| Create | `app/album-guide/page.tsx` | 5ステップのアルバムガイドページ（静的） |

---

### Task 1: `app/album-guide/page.tsx` を作成する

**Files:**
- Create: `app/album-guide/page.tsx`

- [ ] **Step 1: ファイルを作成する**

`app/album-guide/page.tsx` を以下の内容で作成する。
  ...
  },
  {
    ...
    body: [
      ...
      },
      {
        heading: "音源URLの取得方法",
        content:
          "LIFAIの音楽生成ページで曲を生成した後、再生プレイヤー下部に表示される「URLをコピー」ボタンを使って音源URLを取得してください。このURLをnarasu代行フォームに貼り付けます。",
      },
      {
        heading: "楽曲タイトルの決め方",
        content:
...
- [ ] **Step 2: 開発サーバーを起動してページを確認する**

```bash
npm run dev
```

ブラウザで `http://localhost:3000/album-guide` を開き、以下を確認する:
- 5つのステップカードが表示されること
- Step 5 のチェックリストが正しく表示されること
- 「← narasu代理申請に戻る」リンクが表示されること
- モバイル幅（375px）でも崩れないこと

- [ ] **Step 3: コミットする**

```bash
git add app/album-guide/page.tsx
git commit -m "feat: アルバムガイド隠しページを追加 (/album-guide)"
```
...
**Spec coverage:**
- [x] パス `/album-guide` → `app/album-guide/page.tsx` で実装
- [x] 隠しページ（他ページからリンクなし）→ リンク追加なし
- [x] ライト系デザイン → `bg-slate-50 text-slate-900`
- [x] ステップ形式（Step 1〜5）→ `STEPS` 配列で実装
- [x] Step 1: アーティスト名（3表記）
- [x] Step 2: アルバム名（3表記）
- [x] Step 3: 収録曲・音源URL
- [x] Step 4: ジャケット画像（3000×3000px、無料ツール）
- [x] Step 5: 申請前チェックリスト
- [x] narasu代行フォームのフィールド名を対応表示
- [x] フッターに `/narasu-agency` へのリンク

**Placeholder scan:** なし

**Type consistency:** `STEPS` 配列の型は body 内の各オブジェクトに `content` / `list` / `checklist` の optional キーを使い分けており、レンダリング側で `"content" in section` で分岐しているため一貫性あり。
