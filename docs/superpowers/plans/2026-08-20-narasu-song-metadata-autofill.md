...
**⚠️ 注意（他コミットの巻き添えデプロイ）**: `gas/Code.gs` は手動デプロイのため、この変更をデプロイすると、当時ファイルに含まれている未デプロイの他の変更も一緒に本番へ反映される。2026-08-20時点で特に `3f1984e feat(music-boost): クレカ購入の自動有効化＋期限切れ自動処理＋無料有効化の穴を封鎖 (C-1 Phase1)` が未デプロイのまま含まれており、これは支払い経路に影響する変更。この変更を本番稼働させる意図がある場合は、デプロイ後に `setupMusicBoostTriggers()`（`gas/Code.gs:11988`、日次の `musicBoostExpireTrigger_` を設置）を必ず実行すること。意図せず有効化したくない場合は、デプロイ前に `gas/Code.gs` の差分を確認すること。
